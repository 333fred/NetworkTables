...
namespace NetworkTables.Core.Native
{
    ...
    internal class Interop
    {
        ...
        ~Interop()
        {
            ...
            nativeLoader.LibraryLoader.UnloadLibrary(nativeLoader.LibraryHandle);
        ...
        static Interop()
        {
            if (!s_libraryLoaded)
            {
                try
                {
                    ...
                    {
                        nativeLoader = new NativeLibraryLoader(false);
                        nativeLoader.LoadNativeLibrary<Interop>(s_libraryLocation);
                    }
                    else if (File.Exists("/usr/local/frc/bin/frcRunRobot.sh"))
                    {
                        nativeLoader = new NativeLibraryLoader(true);
                        // RoboRIO
